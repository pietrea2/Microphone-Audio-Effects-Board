 ...
 */
std::string generate_header(std::vector<int16_t>& in_buffer, int bytes_per_line)
{
    std::string header;
    header.append("#pragma once\n");
    header.append("#include <cstdint>\n\n");
    header.append("const int16_t image[] = \n");
    header.append("{");
    ...
    for (auto current_char : in_buffer)
    {
        ...
        char hex_string[64];
        sprintf_s(hex_string, "0x%04X", current_char);
...
int main(int argc, char** argv)
{
   ...
   while (fread(bytes, 2, 1, fp) != 0) {
       data.push_back((int16_t)(((uint16_t)bytes[1] << 8) | bytes[0]));
   }
